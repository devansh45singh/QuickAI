    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700 '>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg bg-white rounded-lg border border-gray-200 p-4'>
        <div className='flex items-center  gap-3'>
          ...
          <h1 className='texl-xl font-semibold'>AI Image Generator</h1>
        </div>
      ...
      <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11 '>
        {imageStyle.map((item)=> (
          <span onClick={()=> setSelectedStyle(item)}
           className={`text-xs px-4 py-1 border border-gray-700 rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-100 text-green-700' : 'text-gray-500 border-r-gray-300'} `}
            key={item}>{item}</span>
        ) )}
      </div>
